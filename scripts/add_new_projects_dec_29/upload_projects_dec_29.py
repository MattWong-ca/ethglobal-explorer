...
def upload_new_projects():
    ...
    # Read the JSON file
    with open('new_delhi_online_arg_projects.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
    
    # Filter projects for "new_event_name" event
    all_projects = data['projects']
    new_projects = [project for project in all_projects if project['event'] == 'ETHGlobal Buenos Aires']
